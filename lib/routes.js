        ...
        if (!userParam) {
            ...
            if (req.isAuthenticated()) {
                User.findOne({'username': req.user.username}, function (err, user) {
                    Image.find({'user_id': user.id}, function (err, images) {
                        res.render('profile', {username: req.user.username, images: images, ownPage: true, loggedIn: true});
                    });
                })
            } else {
                res.redirect('/');
            ...
            if (req.isAuthenticated()) {
                ...
                console.log("Parameter is given and User is authenticated");
                ...
                User.findOne({'username': userParam}, function (err, user) {
                    Image.find({'user_id': user.id}, function (err, images) {
                        res.render('profile', {username: userParam, images: images, loggedIn: true, ownPage: ownPage});
                    });
                })
            } else {
                User.findOne({'username': userParam}, function (err, user) {
                    Image.find({'user_id': user.id}, function (err, images) {
                        res.render('profile', {username: userParam, images: images, loggedIn: false});
                    });
                })
            }
        }
...
//                    res.render('profile', {username: req.user.username, images: images, loggedIn: true});
//                });
//            }
//        } else {
//            User.findOne({'username': user}, function (err, user) {
//                Image.find({'user_id': user.id}, function (err, images) {
//                    res.render('profile', {username: user.username, images: images, loggedIn: false});
//                });
//            })
//        }
//    });
